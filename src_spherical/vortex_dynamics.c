...
void rk4_step(struct tangle_state *result, double t, double dt)
{
    ...
    for(int k=0; k < 3; ++k)
    {
        ...
        rk_state[k].domain_section = tangle->domain_section;
...
int do_reconnection(struct tangle_state* tangle, size_t k, size_t l)
{
	...
	#define PSEG(x,y) seg_pwrap(tangle->vnodes+x, tangle->vnodes+y, &tangle->domain_section)
    ...
    for(k = 0; k<tangle->N; ++k)
    {
    	...
    	for(l=k+1; l<tangle->N; ++l)
		{
            if(tangle->status[l].status == EMPTY        ||
	  		...
	  		struct segment seg = seg_pwrap(tangle->vnodes + k, tangle->vnodes + l, &tangle->domain_section);
	  		...
	  		update_one_velocity(tangle, k, t);
			update_one_velocity(tangle, l, t);
