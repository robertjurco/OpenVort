...
void check_distance(struct tangle_state* tangle, struct vec3 r1, struct vec3 r2) {
	struct segment seg = seg_pwrap(&r1, &r2, &tangle->domain_section);
    ...
    for (int i = 0; i < tangle->N; i++) {
        ...
        if (starting_point == -1)
		{
            ...
            for (int j = 1; j < length; j++) {
                ...
                seg = seg_pwrap(&k_old, tangle->vnodes + k, &tangle->domain_section);
                x[j] = seg.r2.p[0];
                y[j] = seg.r2.p[1];
                z[j] = seg.r2.p[2];
			...
			if (vec3_len(&start_to_end) > (tangle->domain_section.inner_radius * sin(tangle->domain_section.polar_angle) * sin(tangle->domain_section.azimut_angle)))
			{
