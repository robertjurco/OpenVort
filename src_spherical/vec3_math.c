...
{
	double x = radius * cos(azim_angle) * cos(polar_angle);
	double y = radius * cos(azim_angle) * sin(polar_angle);
	double z = radius * sin(azim_angle);

	struct vec3 out = vec3(x, y, z);
...
{
	double r = radius(v);
	double azim = azimut_angle(v);
	double polar = polar_angle(v);

	if (r == 0)
	{
		printf("ERROR in spherical_inversion(): Trying to spherically inverse a null vector.");
		exit(EXIT_FAILURE);
	}

	r = radius_of_inversion * radius_of_inversion / r;
...
void print_spherical(struct vec3* v)
{
	...
	double polar = 180 / M_PI * polar_angle(v);

	printf("radius: %g, azimut %g, polar %g\n", r, azim, polar);
}
