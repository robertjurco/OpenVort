...
// General properties.
int frame_shot = 20; // How often to save a snapshot of the tangle.
...
double rec_dist_with_walls = 4e-4; // Recconection distance with walls.
...
void setup_init_conditions(struct tangle_state *tangle)
{
    ...
    //setup domain size
	tangle->domain_section.inner_radius = 0.02;
	tangle->domain_section.outer_radius = 0.2;
	tangle->domain_section.azimut_angle = DEG2RAD(30);
	tangle->domain_section.polar_angle = DEG2RAD(30);

    //add vortices
    if (!load_tangle_from_init) 
	{

		struct vec3 center1 = vec3(0.15, 0.01, 0.05);
		struct vec3 dir1 = vec3(1, 0, 0);
		add_loop(tangle, &center1, &dir1, 0.04);

		struct vec3 center2 = vec3(0.135, 0.0, 0.05);
		struct vec3 dir2 = vec3(0, 1, 0);
		add_loop(tangle, &center2, &dir2, 0.02);

		struct vec3 center3 = vec3(0.1, 0.03, -0.05);
		struct vec3 dir3 = vec3(0, -1, 0);
		add_loop(tangle, &center3, &dir3, 0.02);

		struct vec3 center4 = vec3(0.075, -0.03, -0.05);
		struct vec3 dir4 = vec3(-1, 0, 1);
